...
class EditProductService extends StatefulWidget {
  String id;

  EditProductService({super.key, required this.id});

  @override
  State<EditProductService> createState() => _EditProductServiceState();
}

class _EditProductServiceState extends State<EditProductService> {
              ...
              child: GridView.builder(
                  ...
                  itemBuilder: (BuildContext ctx, index) {
                    if(index == imagebytes.length){
                      ...
                      return rawImages(imagebytes[index], imageId[index]);
                    }
                  }),
            ) : DottedBorder(
              borderType: BorderType.RRect,
  ...
  rawImages(String bytes, String id){
    return Container(
      height: 120,
      child: Stack(
        children: [
      ...
      if (internet) {
        ...
        try {
          ...
          if (resultModel.error == false) {
            setState(() {
              _isLoading = false;
              title= resultModel.allProductService.first.title;
              description = resultModel.allProductService.first.description;
              _descController.text = description == null ? "" : description!;
              _titleController.text = title == null ? "" : title!;

              for(int i=0;i<resultModel.allProductService.first.images.length;i++){
                imagebytes.add(resultModel.allProductService.first.images.elementAt(i).url);
                imageId.add(resultModel.allProductService.first.images.elementAt(i).id.toString());
              }
  ...
  void deleteRequirments(String id) {
    ConnectionUtils.checkConnection().then((internet) async {
      if (internet) {
        ...
        try {
          final ResultModel resultModel = await ApiManager.deleteimageProductService(
            id,
          );
  ...
  void addRequirments() {
    ConnectionUtils.checkConnection().then((internet) async {
      if (internet) {
        ...
        try {
          final ResultModel resultModel = await ApiManager.UpdateProductService(
            widget.id,
